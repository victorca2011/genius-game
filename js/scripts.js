...
const padListener = (e) => {
	...
	setTimeout(() => {
		...
		const curreentMove = _data.playerSequence.length -1;
	
		if (_data.playerSequence[curreentMove] !== _data.gameSequence[curreentMove]) {
			disablePads();
			resetOrPlayAgain();
		} else if (curreentMove === _data.gameSequence.length - 1) {
			newColor();
			playSequence();
		}
		waitForPlayerClick();
